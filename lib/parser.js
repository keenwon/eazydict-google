...
const { EDOutput, Translate, CODES } = require('eazydict-standard-output')
...
// 过滤无效翻译，例如：asdf 会被翻译为 asdf
function filterTranslate(translateList, words) {
  if (translateList && translateList.length === 1 && translateList[0].trans === words) {
    return []
  }
...
function parser(data, words) {
  ...
  if (data[1] && data[1].length) {
    ...
    data[1].forEach((item) => {
      const type = item[0]
      const tran = item[1].slice(0, 6).join(', ')
      const translate = new Translate(type, tran)
    ...
    data[5][0][2].forEach((item) => {
      const tran = item[0]
      const translate = new Translate('', tran)
